<template>
  ...
  >
    <div class="neW" v-bind:class="{ expanded: expand }">
      ...
      <div class="eWHead" v-bind:class="{ edit: edit, expanded: expand }">
...
export default {
  ...
  methods: {
    init() {
      this.mode =
        this.expense && this.expense.id ? (this.edit ? "edit" : "view") : "add";
      this.initPersons();
    },
          ...
          for (var i = 0; i < this.personCollection.length; i++) {
            if (id === this.personCollection[i].id) {
              ...
              this.personCollection[i].modify = {
                mode: "add",
                toPay:
                  this.personCollection[i].toPay + expense.persons[id].cost,
